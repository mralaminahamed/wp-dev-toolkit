...
const HookInspector: React.FC = () => {
  ...
  const getHookTypeBadge = (type: string, count: number) => {
    return (
      <span className={`wdt-inline-flex wdt-items-center wdt-gap-1 wdt-px-2.5 wdt-py-0.5 wdt-rounded-full wdt-text-xs wdt-font-medium ${
        type === 'action' 
        ? 'wdt-bg-green-100 wdt-text-green-800' 
        : 'wdt-bg-blue-100 wdt-text-blue-800'
      }`}>
        {type} <span className="wdt-bg-white wdt-px-1.5 wdt-py-0.5 wdt-rounded-full">{count}</span>
      </span>
    );
  };
    ...
    <div className="wp-dev-toolkit-hook-inspector">
          ...
          <div className="wp-dev-toolkit-dashboard-stat">
            ...
            <div className="wp-dev-toolkit-dashboard-stat-content">
              ...
              <div className="wdt-mt-2 wdt-text-sm wdt-text-gray-500">
                Average: {(hookData.summary.total_executions / hookData.summary.total_hooks).toFixed(2)} per hook
              </div>
            </div>
          </div>
          <div className="wp-dev-toolkit-dashboard-stat">
            ...
            <div className="wp-dev-toolkit-dashboard-stat-content">
              ...
              <div className="wdt-mt-2 wdt-text-sm wdt-text-gray-500">
                Average: {(hookData.summary.total_time / hookData.summary.total_hooks).toFixed(4)} ms per hook
              </div>
            </div>
          </div>
        </div>
      ...
      {isFetching ? (
        ...
        <>
          {/* Hook Details View */}
          {selectedHook && (
            <div id="hook-details" className="wp-dev-toolkit-card wdt-mb-6">
              <div className="wp-dev-toolkit-card-header">
                <div className="wdt-flex wdt-justify-between wdt-items-center">
                  <div className="wdt-flex wdt-items-center">
                    ...
                    <span className={`wdt-ml-2 wdt-px-3 wdt-py-1 wdt-text-xs wdt-rounded-full wdt-font-medium ${getHookTypeClass(selectedHook.type)}`}>
                      {selectedHook.type}
                    </span>
                  </div>
                  <Button
                ...
                <div className="wdt-overflow-x-auto">
                  <table className="wdt-w-full wdt-border wdt-border-gray-200 wdt-rounded-lg wdt-overflow-hidden">
                    <thead className="wdt-bg-gray-50 wdt-border-b wdt-border-gray-200">
                      <tr>
                        <th className="wdt-py-3 wdt-px-4 wdt-text-left wdt-text-xs wdt-font-medium wdt-text-gray-500 wdt-uppercase wdt-tracking-wider">Priority</th>
                        <th className="wdt-py-3 wdt-px-4 wdt-text-left wdt-text-xs wdt-font-medium wdt-text-gray-500 wdt-uppercase wdt-tracking-wider">Function</th>
                        <th className="wdt-py-3 wdt-px-4 wdt-text-left wdt-text-xs wdt-font-medium wdt-text-gray-500 wdt-uppercase wdt-tracking-wider">Location</th>
                        <th className="wdt-py-3 wdt-px-4 wdt-text-left wdt-text-xs wdt-font-medium wdt-text-gray-500 wdt-uppercase wdt-tracking-wider">Args</th>
                      </tr>
                    </thead>
                    <tbody className="wdt-bg-white wdt-divide-y wdt-divide-gray-200">
                      {selectedHook.callbacks.map((callback: HookCallback, index: number) => (
                        <tr key={index} className="hover:wdt-bg-gray-50">
                          <td className="wdt-py-3 wdt-px-4 wdt-font-medium">{callback.priority}</td>
                          ...
                          <td className="wdt-py-3 wdt-px-4 wdt-text-xs wdt-truncate wdt-max-w-[200px]">
                            <div className="wdt-flex wdt-items-center wdt-gap-1">
                              <Dashicon icon="admin-page" size={14} className="wdt-text-gray-400" />
                              <span title={callback.file}>{formatFilePath(callback.file)}</span>
                              <span className="wdt-text-gray-400">:</span>
                              <span className="wdt-text-gray-700">{callback.line}</span>
                            </div>
                          </td>
                          <td className="wdt-py-3 wdt-px-4">{callback.accepted_args}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <thead className="wdt-bg-gray-50 wdt-border-b wdt-border-gray-200">
                    <tr>
                      ...
                      >
                        <div className="wdt-flex wdt-items-center">
                          <span>Time (ms)</span>
                          <span className="wdt-ml-1">{sortIndicator('total_time')}</span>
                        </div>
                      </th>
                    ...
                    {sortedHooks.length > 0 ? (
                      sortedHooks.map((hook, index) => (
                        <tr key={index} className="hover:wdt-bg-gray-50 wdt-transition-colors">
                          <td className="wdt-py-3 wdt-px-4 wdt-font-medium wdt-font-mono wdt-text-sm">{hook.name}</td>
                          <td className="wdt-py-3 wdt-px-4">
                            <span className={`wdt-px-2 wdt-py-1 wdt-text-xs wdt-rounded-full ${getHookTypeClass(hook.type)}`}>
                              {hook.type}
                            </span>
                          </td>
                      ...
                      <tr>
                        <td colSpan={5} className="wdt-py-8 wdt-px-4 wdt-text-center wdt-text-gray-500">
