              ...
              ( tab.name !== 'dashboard' && currentPath === `/${ tab.name }` );
						...
						return (
							<li key={ tab.name } className="wdt:relative">
								{ isActive && (
									<div className="wdt:absolute wdt:left-0 wdt:top-0 wdt:h-full wdt:w-1 wdt:bg-blue-600 wdt:rounded-r"></div>
								) }
								<Link
									...
									className={ `wdt:flex wdt:items-center wdt:gap-3 wdt:px-4 wdt:py-3 wdt:text-gray-700 wdt:no-underline wdt:rounded-md wdt:transition-all wdt:duration-150 ${
										isActive
											? 'wdt:bg-blue-50 wdt:text-blue-700'
											: 'wdt:hover:bg-gray-50 wdt:hover:text-blue-600'
									}` }
								>
									<span className="wdt:text-current wdt:text-lg">
										{ tab.icon }
									</span>
